...
export default function HelpModal({ onClose, onReplayTutorial }: HelpModalProps) {
  ...
  return (
    <div className="absolute inset-0 z-50 bg-goshiwon-bg/95 backdrop-blur-sm flex flex-col animate-vocab-panel-in">
      ...
      {/* Tab bar */}
      <div className="flex gap-1 px-3 py-2 border-b border-goshiwon-border overflow-x-auto">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`shrink-0 px-3 py-1.5 rounded-full text-[11px] transition-colors ${
              activeTab === tab.id
          ...
          >
            {tab.label}
            <span className="ml-1 text-[9px] opacity-60">{tab.labelEn}</span>
          </button>
        ))}
      </div>
