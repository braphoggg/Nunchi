...
export default function WelcomeScreen({ onSelectTopic, rank, visitedTopics, dueCount = 0, onStartStudy }: WelcomeScreenProps) {
  ...
  return (
    <div className="w-full flex flex-col items-center px-3 sm:px-4 py-1 sm:py-8">
      ...
      <div data-tutorial="welcome-topics" className="grid grid-cols-2 gap-1 sm:gap-3 w-full max-w-lg">
        {LESSON_TOPICS.map((topic, index) => {
          ...
          return (
            <button
              ...
              className={`group relative text-left p-1.5 sm:p-4 bg-goshiwon-surface border border-goshiwon-border rounded-lg transition-all duration-300 border-t-2 ${
      ...
      <p className="hidden sm:block mt-8 text-xs text-goshiwon-text-secondary text-center max-w-sm">
